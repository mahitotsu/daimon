...
Claude Code に日本語で「今日はどれくらい作業した?」と尋ねると、`claude-observability` Skill が Prometheus に問い合わせて要約を返しつつ、詳細確認用に「利用サマリー」ダッシュボードへのリンクも返す(2026-07-06、Claude Code v2.1.201 で確認):

![Claude Code 利用サマリーダッシュボードの例](docs/images/daimon_proto_20260707.png)
*Claude Code v2.1.201 / 2026-07-06 時点の例。表示されているコスト・トークン数は実データ。*
...
| `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA` | `1` | trace(span)出力を有効化するベータフラグ。プレーン名のみで CLI・VS Code 拡張の両方に効く(下記参照) |
...
| `OTEL_TRACES_EXPORTER` | `otlp` | 同上。span(`claude_code.interaction`/`llm_request`/`tool`/`tool.execution` 等)の送出に必要 |
...
| `ANT_OTEL_TRACES_EXPORTER` | `otlp` | 同上 |
...
**トレース(ベータ)を有効にして Tempo に呼び出し系列・所要時間を送る**: 公式ドキュメント([Monitoring - Claude Code Docs](https://code.claude.com/docs/en/monitoring-usage))によれば、`CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` + `OTEL_TRACES_EXPORTER=otlp` で `claude_code.interaction`(ターン)→ `claude_code.llm_request`(モデル呼び出し)/ `claude_code.tool`(ツール呼び出し、`tool_use_id` 付き)→ `claude_code.tool.execution`・`claude_code.tool.blocked_on_user` という span 階層が出力される。`tool_use_id`/`gen_ai.tool.call.id` は JSONL 側の `tool_result`/`tool_decision` と同じ ID なので、span 側は呼び出し系列・回数・所要時間の集計、内容(プロンプト本文・ツール引数)は引き続き JSONL 側、という役割分担がそのまま成立する。prompt/tool 入出力の内容はここでも既定でリダクトされ、`OTEL_LOG_USER_PROMPTS`/`OTEL_LOG_TOOL_DETAILS`/`OTEL_LOG_TOOL_CONTENT` を立てない限り載らない。

`CLAUDE_CODE_ENHANCED_TELEMETRY_BETA` はプレーン名しか存在せず(`ANT_` 版はない)、VS Code 拡張機能の組み込みバイナリでもプレーン名のまま効く(`CLAUDE_CODE_ENABLE_TELEMETRY` と同じ扱い、native-binary/claude の `strings` で確認)。`OTEL_TRACES_EXPORTER` は他の exporter と同様プレーン名/`ANT_` 版の両方が必要。既存の `OTEL_EXPORTER_OTLP_ENDPOINT`/`PROTOCOL`(および `ANT_` 版)がそのまま traces にも使われるため、trace 専用のエンドポイント変数は設定していない。2026-07-07、Claude Code v2.1.202 で `claude -p` からの実 API 呼び出しを1回行い、Tempo に上記の span 階層が実際に届くことを実機確認済み。同日、VS Code 拡張機能のチャットパネル経由(entrypoint `claude-vscode`)でも、`claude_code.llm_request`/`tool`/`tool.execution`/`tool.blocked_on_user` が実際に Tempo に届くことを実機確認した(`claude_code.interaction` はターン終了時にしか閉じないため、ターン進行中は未着のまま見えるのが正常)。

ベータ機能のため将来のバージョンで挙動が変わる可能性がある。既知の関連問題として、Agent SDK の `query()` / ACP streaming-json 経由では `claude_code.interaction`/`tool` span が欠落し `llm_request` のみになる不具合が報告されている([anthropics/claude-code#53954](https://github.com/anthropics/claude-code/issues/53954)、Closed as not planned)。これは Agent SDK 固有のパスの話であり、本ツールが対象とする通常の CLI・VS Code 拡張機能には影響しないと見られるが、上記の未検証部分と合わせて留意すること。
