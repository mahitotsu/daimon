...
(Editing the dashboard JSON itself -- Tempo panel query schema, provisioning hot-reload -- is a maintenance concern, not a runtime Q&A one; see CLAUDE.md.)
...
Tempo traces are the native fit for frequency/duration/error-rate questions specifically, since they carry duration and status as first-class span fields rather than an attribute needing `unwrap` -- don't reintroduce custom parsing or prefer LogQL here when TraceQL covers it directly (see CLAUDE.md for why this replaced the earlier Python-script/Loki-only approach).
...
**Put this caveat in the answer, not just in your own reasoning.** Confirmed 2026-07-09: a response that gave a confident-sounding breakdown with no mention of this method's blind spot or sample size scored worse on review than one that stated the same numbers alongside "this misses silent corrections, and n=20 is small" -- knowing the limitation yourself doesn't help the user unless they're told it too.
...
**When the answer hinges on a specific date or the order of past events, don't state it from memory of the transcript prose -- verify it against a timestamp.** Confirmed 2026-07-09: asked to reconstruct when a past decision/file change happened, a response stated dates that were off by a day or two against what `git log` actually shows for the relevant commits. Conversation prose ("the other day", "after that") is easy to misremember when synthesizing across multiple sessions; each Loki line's own timestamp and `git log` (for anything that ended up committed) are the ground truth -- check one of those before asserting a specific date.
